...
#define HL_HIGHLIGHT_ENCLOSINGS (1<<2)
/*** data ***/
...
int alreadyDied = 0; // prevent recursive dying
...
char *C_HL_keywords[] = {
  ...
  "void|", "#define|", "#include|", "const|", NULL
};
...
struct editorSyntax HLDB[] = {
  {
    ...
    C_HL_keywords,
    "//", "/*", "*/",
    HL_HIGHLIGHT_NUMBERS | HL_HIGHLIGHT_STRINGS | HL_HIGHLIGHT_ENCLOSINGS
  }
};
...
void editorUpdateSyntax(erow *row) {
  ...
  while ( i < row->rsize) {
    ...
    if (E.syntax->flags & HL_HIGHLIGHT_STRINGS) {
      if (in_string) {
        row->hl[i] = HL_STRING;
        if (c == '\\' && i + 1 < row->rsize) { // Escape Characters
    ...
    // Highlights enclosing and function calls
    if (E.syntax->flags & HL_HIGHLIGHT_ENCLOSINGS && is_enclosing(c)) {
      row->hl[i] = HL_ENCLOSINGS;
      if (c == '(') {
        int k = i - 1;
        while ( k >= 0 && !is_separator(row->render[k])) {
          row->hl[k] = HL_FUNCTIONS;
          k -= 1;
        }
      }
    }
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx ++) {
    if (row->chars[cx] == '\t')
      cur_rx += (NOTVSCODE_TAB_STOP - 1) - (cur_rx % NOTVSCODE_TAB_STOP);
    cur_rx++;
...
void editorRowInsertChar(erow *row, int at, int c) {
  ...
  row->chars = realloc(row->chars, row->size + 2); // Null Byte and extra space for extra character
  memmove(&row->chars[at + 1], &row->chars[at], row->size - at + 1); // Because we are shfiting data on same obj
  row->size++;
  row->chars[at] = c;
  editorUpdateRow(row);
  E.dirty++;
...
char *editorRowsToString(int *buflen) {
  ...
  *buflen = totlen;
  ...
  char *p = buf;
  for (j = 0; j < E.numrows; j++) {
    memcpy(p, E.row[j].chars, E.row[j].size); // We dont copy the Null Terminator
    p += E.row[j].size;
    *p = '\n';
    p++;
  }
...
void editorDrawStatusBar(struct abuf *ab) {
  ...
  int rlen = snprintf(rstatus, sizeof(rstatus), "%s | %d/%d",
    E.syntax ? E.syntax->filetype : "no ft", E.cy + 1, E.numrows); // For Current Line Number in File
...
void editorProcessKeypress(){
  static int quit_times = NOTVSCODE_QUIT_TIMES;
    ...
    case CTRL_KEY('k'):
      if (E.dirty && quit_times > 0) {
        editorSetStatusMessage("WARNING!!! File has unsaved changes. "
          "Press Ctrl-K %d more times to quit.", quit_times);
          quit_times--;
          return;
      }
      write(STDOUT_FILENO, "\x1b[2J", 4);
  ...
  quit_times = NOTVSCODE_QUIT_TIMES;
}
...
main(int argc, char *argv[]) {
  ...
  editorSetStatusMessage("HELP: Ctrl-S = save | Ctrl-K = quit | Ctrl-N = find");
